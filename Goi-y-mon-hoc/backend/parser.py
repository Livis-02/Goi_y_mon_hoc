...
def parse_xlsx_bytes(data: bytes) -> List[List[Any]]:
    df = pd.read_excel(io.BytesIO(data), sheet_name=0, engine="openpyxl")
    return [list(df.columns)] + df.values.tolist()
def parse_csv_bytes(data: bytes) -> List[List[Any]]:
...
def parse_pdf_bytes(data: bytes) -> List[List[Any]]:
    ...
    return text_rows
def read_rows_from_upload(filename: str, data: bytes) -> List[List[Any]]:
...
def extract_tich_luy(rows: List[List[Any]]) -> "float | None":
    """Extract 'Số tín chỉ tích lũy' value from transcript rows."""
    for row in rows:
        if not row:
            continue
        for i, cell in enumerate(row):
            ...
            if "so tin chi tich luy" in cell_text:
                # Value may be in same cell after ":", e.g. "Số tín chỉ tích lũy: 144"
