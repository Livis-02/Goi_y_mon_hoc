...
_SPEC_DOCX_TO_DISPLAY: dict[str, str] = {
    "cntt dia hoc": "7480201_08",
    "cong nghe thong tin dia hoc": "7480201_08",
    "khoa hoc may tinh": "7480201_07",
    "tin hoc kinh te": "7480201_04",
    "mang may tinh": "7480201_06",
    "he thong thong tin": "7480201_09",
    "cong nghe phan mem": "7480201_05",
}

_GROUP_MIN_CREDITS: dict[str, float] = {"A": 6.0, "B": 9.0, "C": 9.0}

_NON_COUNTING_CODES: frozenset[str] = frozenset({
    "7010701", "7010702", "7010703",          # GDTC 1/2/3
    "7300101", "7300102", "7300201",          # QPAN chính thức (HK4/5/6)
    "7300103", "7300104", "7300202", "7300203",  # QPAN chứng chỉ GDQP
    "7010610",                                 # Tiếng Anh tăng cường
})
...
def _parse_tabular(
    ...
    spec_col = _find_col(list(df.columns), ["chuyen nganh", "specialization", "nganh", "required_specialization"])
    ...
    for _, row in df.iterrows():
        ...
        req_spec: str | None = None
        if spec_col:
            sv = str(row.get(spec_col) or "").strip()
            # Accept both spec codes (7480201_07) and normalised Vietnamese names
            if re.fullmatch(r"\d{7}_\d{2}", sv):
                req_spec = sv
            elif sv:
                req_spec = _lookup_spec(sv)
        seen[code] = (code, name, credits, req_spec)

        if group_col:
            group_val = str(row.get(group_col) or "").strip()
            if group_val:
                spec_for_elective = req_spec
                elective_mappings.append((code, spec_for_elective, group_val.upper()))
...
def import_curriculum_from_excel(
    ...
    *,
    ...
    for entry in parsed:
        ...
        counts = code not in _NON_COUNTING_CODES
        ...
        if existing:
            ...
            existing.count_toward_credits = counts
            updated_courses += 1
        else:
            db.add(models.Course(
                course_code=code, course_name=name, credits=credits,
                required_specialization=req_spec, count_toward_credits=counts,
            ))
            inserted_courses += 1
