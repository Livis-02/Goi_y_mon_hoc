...
function initCurriculumChart(containerEl, opts) {
  ...
  function _makeCol(label, hkNum, list) {
    ...
    col.style.cssText =
      `display:flex;flex-direction:column;gap:6px;width:168px;flex-shrink:0;position:relative;z-index:2;` +
      (dimmed ? 'opacity:.65' : '');

    const hdr = document.createElement('div');
    hdr.style.cssText =
      `text-align:center;padding:6px 8px;border-radius:8px;` +
      (dimmed ? 'background:#94a3b8' : 'background:#334155') +
      `;color:#fff;user-select:none;`;
    hdr.innerHTML =
      `<div style="font-size:11px;font-weight:700;letter-spacing:.3px">${_esc(label)}</div>` +
      `<div style="font-size:9px;color:rgba(255,255,255,.65);margin-top:1px">${list.length} môn · ${tcStr} TC</div>`;
    col.appendChild(hdr);
  ...
  function _makePlaceholderCard(c) {
    ...
    card.style.cssText =
      `position:relative;border:2px dashed ${clr.border};background:${clr.bg};border-radius:8px;` +
      `padding:8px 10px;cursor:default;opacity:.7;`;
    card.innerHTML =
      `<div style="font-size:11px;font-style:italic;font-weight:600;color:${clr.code};text-align:center">${label}</div>` +
      `<div style="font-size:10px;color:#94a3b8;margin-top:2px;text-align:center">${c.credits || 3} TC</div>`;
    return card;
  }
  ...
  function _makeCard(c) {
    ...
    card.style.cssText =
      `position:relative;border-left:3px solid ${borderColor};background:${bgColor};border-radius:8px;` +
      `padding:7px 9px;cursor:${canEdit ? 'grab' : 'pointer'};` +
      `transition:opacity .15s,box-shadow .15s,outline .1s;` +
      `box-shadow:0 1px 2px rgba(0,0,0,.06);user-select:none;`;

    const badgeHtml = st
      ? `<span style="position:absolute;top:4px;right:24px;font-size:9px;font-weight:700;color:${st.bclr};background:${st.bbg};border-radius:3px;padding:0 3px;line-height:1.3">${st.badge}</span>`
      : '';

    const noteBadge = c.has_note
      ? `<span style="position:absolute;top:4px;right:4px;font-size:10px" title="Cố vấn có ghi chú">💬</span>`
      : '';

    card.innerHTML =
      `${badgeHtml}${noteBadge}` +
      `<div style="font-family:monospace;font-size:10px;font-weight:700;color:${clr.code};margin-bottom:2px;padding-right:18px">${_esc(c.course_code)}</div>` +
      `<div style="font-size:11.5px;color:#1e293b;line-height:1.3;word-break:break-word">${_esc(c.course_name)}</div>` +
      `<div style="font-size:10px;color:#94a3b8;margin-top:3px">${c.credits != null ? c.credits + ' TC' : '—'}${!c.count_toward_credits ? ' <span style="color:#d97706;font-weight:600">-TC</span>' : ''}</div>` +
      (canEdit ? `<div class="cc-port" data-code="${_esc(c.course_code)}" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);width:12px;height:12px;border-radius:50%;background:#fff;border:2px solid ${borderColor};cursor:crosshair;display:none;box-shadow:0 1px 2px rgba(0,0,0,.2)" title="Kéo để nối tiên quyết"></div>` : '');
